import type { Card, HandKey, RawAction, RuleSet, StrategyEntry } from '@/lib/types'
import { strategyRank } from './card'
import { buildStrategyTable, lookupEntry } from './strategyTable'
import type { StrategyTableMap } from '@/lib/types'
...
export function actionCategory(raw: RawAction): import('@/lib/types').DecisionCategory {
  switch (raw) {
    case 'hit': return 'hit'
    case 'stand': return 'stand'
    case 'doubleOrHit':
    case 'doubleOrStand': return 'double'
    case 'split': return 'split'
    case 'surrenderOrHit':
    case 'surrenderOrStand':
    case 'surrenderOrSplit': return 'surrender'
  }
}
