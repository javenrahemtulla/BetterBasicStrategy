import type {
  ActionRecord, Card, DecisionCategory, GameState,
  Outcome, RuleSet, SpotState,
} from '@/lib/types'
import { makeCard } from './card'
import { buildHandState, calcTotal } from './hand'
import { dealCard, needsReshuffle, createShoe, saveShoe } from './shoe'
...
function makeSpot(id: string): SpotState {
  return {
    id,
    hands: [buildHandState([])],
    activeHandIndex: 0,
    outcomes: [('lose' as Outcome)],
    actions: [[]],
    isComplete: false,
  }
}

function initialState(rules: RuleSet): GameState {
  return {
    phase: 'idle',
    spots: [],
    activeSpotIndex: 0,
...
export function dealNewRound(
  ...
  // 2 passes: player card, dealer card, player card, dealer hole
  ...
  // Mark blackjack spots complete
  for (const spot of spots) {
    if (spot.hands[0].isBlackjack) {
      spot.isComplete = true
      spot.outcomes = [('blackjack' as Outcome)]
    }
  }
...
export function getAvailableActions(state: GameState, rules: RuleSet): Set<DecisionCategory> {
  ...
  // Double: first 2 cards only; after split only if DAS is on
  if (hand.cards.length === 2) {
    const isAfterSplit = spot.hands.length > 1
    if (!isAfterSplit || rules.doubleAfterSplit) available.add('double')
  }

  // Split: 2 cards, same strategy rank, within split limits
  if (hand.cards.length === 2) {
    const { strategyRank } = require('./card')
  ...
  // Surrender: first action only, first 2 cards
  if (
    hand.cards.length === 2 &&
    spot.actions[spot.activeHandIndex].length === 0 &&
...
export function performAction(
  ...
  let s = shoe
  const spots = state.spots.map(sp => ({
    ...sp,
    hands: [...sp.hands],
    actions: sp.actions.map(a => [...a]),
  ...
  const correctCategory = actionCategory(resolved)
  ...
  let newState: GameState = {
    ...
    lastCoachingEntry: isCorrect ? null : (entry ?? null),
  }
  ...
  switch (category) {
    case 'hit': {
      ...
      if (result) {
        ...
        spot.hands[spot.activeHandIndex] = buildHandState(newCards)
        if (spot.hands[spot.activeHandIndex].total >= 21) {
          newState = advanceHand({ ...newState, spots }, s, rules)
          saveShoe(s)
    ...
    case 'split': {
      ...
      const fill1 = dealCard(s)
      if (fill1) s = fill1.shoe
      const fill2 = dealCard(s)
      if (fill2) s = fill2.shoe
      ...
      spot.outcomes = [...spot.outcomes, 'lose' as Outcome]
      // Auto-advance if split aces and no resplit
    ...
    case 'surrender': {
      spot.outcomes = ['surrender']
      spot.isComplete = true
      newState = advanceIfNeeded({ ...newState, spots }, s, rules)
      saveShoe(s)
      return { state: newState, shoe: s }
    }
  }
...
function advanceIfNeeded(state: GameState, shoe: ShoeState, rules: RuleSet): GameState {
  ...
  if (state.spots.every(sp => sp.isComplete)) {
    return playDealer(state, shoe, rules).state
  }
  return state
}
